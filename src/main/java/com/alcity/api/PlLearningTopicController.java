...
public class PlLearningTopicController {
    ...
    @Operation( summary = "Save a puzzle level learning topic  ",  description = "Save a puzzle level learning topic entity and their data to data base")
    @PostMapping("/save")
    @CrossOrigin(origins = "*")
    ...
    @DeleteMapping("/del/content-id/{contentId}/learn-topic/{learningtopicId}")
    @CrossOrigin(origins = "*")
    public ALCityResponseObject deletePLLearningTopicById(@PathVariable Long learningtopicId,@PathVariable Long learningContentId) {
        Optional<LearningTopic> learningTopicOptional = learningTopicService.findById(learningContentId);
        Optional<LearningContent> learningContentOptional = learningContentService.findById(learningtopicId);
        Optional<LearningTopicInPL> existingRecord=null;
        if(learningTopicOptional.isPresent() && learningContentOptional.isPresent()){
            existingRecord = plLearningTopicService.findByLearningTopicAndAndLearningContent(learningTopicOptional.get(),learningContentOptional.get());
            try {
            ...
            return new ALCityResponseObject(HttpStatus.OK.value(), "ok", existingRecord.get().getId(),"Record deleted Successfully!");
        }

        return  new ALCityResponseObject(HttpStatus.NO_CONTENT.value(), "error", learningtopicId + learningtopicId,"Record not found!");
    }
